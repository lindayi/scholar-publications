...
class SchPub_Shortcode {
		...
		<p class="schpub-meta">
			...
			if ( '' !== $orcid ) :
				...
				<a class="schpub-orcid-link" href="<?php echo esc_url( 'https://orcid.org/' . $orcid ); ?>"
					...
					aria-label="<?php
					printf(
						/* translators: %s: ORCID identifier. */
						esc_attr__( 'View ORCID record %s', 'scholar-publications' ),
						esc_attr( $orcid )
					);
					?>">
					<?php self::orcid_icon(); ?><span><?php echo esc_html( $orcid ); ?></span>
				</a>
			<?php endif; ?>
			<?php if ( $updated ) : ?>
	 ...
	 * drawn in the trademarked green in both colour schemes because the icon is
	 * a brand asset rather than themed UI.
	 */
